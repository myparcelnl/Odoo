...
from odoo.addons.delivery_myparcel_official.data.myparcel_static_data import MYPARCEL_DELIVERY_TYPES, \
...
class BaseProviderMyParcel(models.Model):
    ...
    x_aa_mp_platform = fields.Selection([('myparcel', 'MyParcel NL'), ('sendmyparcel', 'MyParcel BE')],
                                        string='MyParcel Platform', store=True)
    ...
    # @api.depends('x_aa_mp_is_myparcel', 'x_aa_mp_is_sendmyparcel')
    # def _set_platform(self):
    #     for rec in self:
    #         if rec.x_aa_mp_is_myparcel and not rec.x_aa_mp_is_sendmyparcel:
    #             rec.x_aa_mp_platform = 'myparcel'
    #         elif rec.x_aa_mp_is_sendmyparcel and not rec.x_aa_mp_is_myparcel:
    #             rec.x_aa_mp_platform = 'sendmyparcel'
    #         else:
    #             rec.x_aa_mp_platform = False
    ...
    # def base_myparcel_return_shipment(self, picking=None, options=None, delivery_type=False):
    #     # TODO: Discuss with Jort if we want to put return on the delivery.carrier model or keep it on stock.picking
    #     # TODO: Relate this method to a return picking and the return picking wizard
    #     # TODO: Implement different options in the return shipment and add options choice to return picking wizard
    #     if picking:
    #         if picking.x_aa_mp_shipping_id:
    #             myparcel_request = self.get_myparcel_request(carrier_id=self,
    #                                                          module_version=self.get_module_version())
    #             result = myparcel_request.return_shipment(picking, options,
    #                                                       self._get_carrier_code(delivery_type=delivery_type),
    #                                                       recipient=picking.partner_id)
    #             return_shipment_id = None
    #             for return_shipment_ids in result.get('data').get('ids'):
    #                 return_shipment_id = return_shipment_ids.get('id')
    #                 _logger.warning(F'return_shipment_id {return_shipment_id}')
    #
    #             # TODO: Store the return_shipment_id in the return picking so that we can print a new label
    #             return return_shipment_id
    #         else:
    #             raise ValidationError(_('No shipment found for this picking. '
    #                                     'Check your MyParcel/SendMyParcel account.'))
    #     else:
    #         # TODO: Generate Unrelated return
    #         raise ValidationError(_('No unrelated return implemented in this version.'))
    #
    #     # TODO: Implement label printing for return shipments and tracking link retrieval

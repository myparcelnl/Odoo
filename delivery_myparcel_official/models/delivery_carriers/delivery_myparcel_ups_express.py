...
class ProviderMyparcelUPSExpress(models.Model):
    ...
    def myparcel_ups_express_check_option_combi(self, wizard_id=None):
        if self.delivery_type == 'myparcel_ups_express':
            if wizard_id:
                if wizard_id.x_aa_mp_age_control and not wizard_id.x_aa_mp_signing:
                    raise ValidationError(
                        _('Age check requires Signature option. Please select the correct options.'))
            else:
                if self.x_aa_mp_age_control and not self.x_aa_mp_signing:
                    raise ValidationError(
                        _('Age check requires Signature option. Please select the correct options.'))

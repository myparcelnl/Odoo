...
class MyParcelRequest:
    ...
    def _get_url(self, request_type, shipment_id=None):
        if request_type in ['add_shipments', 'return_shipments']:
    ...
    def _get_user_agent(self):
        user_agent = f'MyParcel-Odoo/{self.odoo_version} delivery_myparcel_official/{self.module_version}'
        return user_agent
    ...
    def post_shipment_body(self, recipient, options, record=None, carrier_code='', carrier=None):
        ...
        if carrier_code == 1 and record:
            if record._name == 'stock.picking' and record.packages_count > 1:# record.package_level_ids:
                ...
                for sequence, move in enumerate(record.move_ids):
                    if sequence == 0:
                        shipments['reference_identifier'] = (
                            '{picking_name} {package_name}'
                        ).format(**{
                            'picking_name': record.name,
                            'package_name': move.package_ids[0].name,
                        })
                        if len(move.package_ids) > 1:
                            for pack in move.package_ids[1:]:
                                shipments['secondary_shipments'].append({
                                    'reference_identifier': (
                                        '{picking_name} {package_name}'
                                    ).format(**{
    ...
    # TODO: Implement return shipment
    # def post_return_shipment_body(self, recipient, options, record=None, carrier_code='', carrier=None):
    #     # Check if we are making a related or unrelated return shipment
    #     if record.x_aa_mp_shipping_id:
    #         related_return = True
    #     else:
    #         related_return = False
    #
    #     # Get Insurance price
    #     if options and 'insurance' in options:
    #         options = self._get_insurance_price(record, options)
    ...
    #     if related_return:
    #         return_shipments = {
    #             "parent": record.x_aa_mp_shipping_id,
    #             "carrier": carrier_code,
    #             "name": recipient.name,
    ...
    #         }
    #     else:
    #         # TODO: Implement "sender" which contains the address of the original sender/own company (t.b.d.)
    #         return_shipments = {
    #             "carrier": carrier_code,
    #             "name": recipient.name,
    #             "email": recipient.email,
    ...
    #     return data
    #
    # def return_shipment(self, record, options=None, carrier_code='', recipient=None):
    #     # TODO: implement return shipment
    #     headers = self._get_headers('return_shipments')
    #     url_api = self._get_url('return_shipments')
    #
    #     if recipient is None:
    #         recipient = record.partner_id
    #     else:
    ...
    #     if options is None:
    #         options = {}
    #     return_shipment_body = self.post_return_shipment_body(recipient=recipient, options=options, record=record,
    #                                             carrier_code=carrier_code, carrier=self.carrier_id)
    #
    #     _logger.warning(F'url_api {url_api}')
    #     _logger.warning(F'headers {headers}')
    #     _logger.warning(F'shipment_body {return_shipment_body}')
    #
    #     response = requests.post(url_api,
    #                              data=json.dumps(return_shipment_body),
    #                              headers=headers)
    #
    #     json_response = response.json()
    #     _logger.warning(F'response check {json_response}')
